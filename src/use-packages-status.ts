...
export const generalStatusList: GeneralStatus[] = [
  ...
  },
  {
    ...
    value: 'pending',
    children: [PackageStatus.Waiting],
...
export function usePackageStatus(language?: SupportedLocale) {
  ...
  packageStatusListStatic.forEach((detail) => {
    StatusDetailsMap.set(detail.value, { ...detail, label: t(detail.labelKey) })
  })
  StatusDetailsMap.set(0, { ...UNKNOWN_STATUS, label: t(UNKNOWN_STATUS.labelKey) })

  const getStatus = (value?: number): PackageStatusLabelType => {
    const code = (value as PackageStatusType) || PackageStatus.Unknown

    return StatusDetailsMap.get(code) ?? { ...UNKNOWN_STATUS, label: t(UNKNOWN_STATUS.labelKey) }
  }
